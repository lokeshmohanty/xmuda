...
from xmuda.data.nuscenes_lidarseg.nuscenes_lidarseg_dataloader import (
    NuScenesLidarSegSCN,
)
...
def build_dataloader(
    cfg, mode="train", domain="source", start_iteration=0, different_batch_size=None
):
    ...
    if dataset_cfg.TYPE == "NuScenesSCN":
        dataset = NuScenesSCN(
            split=split, output_orig=not is_train, **dataset_kwargs, **augmentation
        )
        ...
        dataset = A2D2SCN(split=split, **dataset_kwargs, **augmentation)
    elif dataset_cfg.TYPE == "SemanticKITTISCN":
        dataset = SemanticKITTISCN(
            split=split, output_orig=not is_train, **dataset_kwargs, **augmentation
        )
    elif dataset_cfg.TYPE == "VirtualKITTISCN":
        dataset = VirtualKITTISCN(split=split, **dataset_kwargs, **augmentation)
    else:
    ...
    if is_train:
        ...
        batch_sampler = BatchSampler(
            sampler, batch_size=batch_size, drop_last=cfg.DATALOADER.DROP_LAST
        )
        batch_sampler = IterationBasedBatchSampler(
            batch_sampler, cfg.SCHEDULER.MAX_ITERATION, start_iteration
        )
