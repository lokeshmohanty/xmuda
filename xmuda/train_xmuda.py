...
def train(cfg, output_dir="", run_name=""):
    ...
    checkpoint_data_2d = checkpointer_2d.load(
        cfg.RESUME_PATH, resume=cfg.AUTO_RESUME, resume_states=cfg.RESUME_STATES
    )
    ...
    set_random_seed(cfg.RNG_SEED)
    train_dataloader_src = build_dataloader(
        cfg, mode="train", domain="source", start_iteration=start_iteration
    )
    train_dataloader_trg = build_dataloader(
        cfg, mode="train", domain="target", start_iteration=start_iteration
    )
    val_period = cfg.VAL.PERIOD
    val_dataloader = (
        build_dataloader(cfg, mode="val", domain="target") if val_period > 0 else None
    )
    ...
    for iteration in range(start_iteration, max_iteration):
        ...
        seg_loss_src_2d = F.cross_entropy(
            preds_2d["seg_logit"], data_batch_src["seg_label"], weight=class_weights
        )
        seg_loss_src_3d = F.cross_entropy(
            preds_3d["seg_logit"], data_batch_src["seg_label"], weight=class_weights
        )
        ...
        if cfg.TRAIN.XMUDA.lambda_xm_src > 0:
            ...
            train_metric_logger.update(
                xm_loss_src_2d=xm_loss_src_2d, xm_loss_src_3d=xm_loss_src_3d
            )
        ...
        if cfg.TRAIN.XMUDA.lambda_xm_trg > 0:
            ...
            train_metric_logger.update(
                xm_loss_trg_2d=xm_loss_trg_2d, xm_loss_trg_3d=xm_loss_trg_3d
            )
        ...
        if cfg.TRAIN.XMUDA.lambda_pl > 0:
            ...
            pl_loss_trg_2d = F.cross_entropy(
                preds_2d["seg_logit"],
                data_batch_trg["pseudo_label_2d"],
                weight=class_weights_pl,
            )
            pl_loss_trg_3d = F.cross_entropy(
                preds_3d["seg_logit"],
                data_batch_trg["pseudo_label_3d"],
                weight=class_weights_pl,
            )
            train_metric_logger.update(
                pl_loss_trg_2d=pl_loss_trg_2d, pl_loss_trg_3d=pl_loss_trg_3d
            )
        ...
        # checkpoint
        if (
            ckpt_period > 0 and cur_iter % ckpt_period == 0
        ) or cur_iter == max_iteration:
            checkpoint_data_2d["iteration"] = cur_iter
            checkpoint_data_2d[best_metric_name] = best_metric["2d"]
            checkpointer_2d.save(
                "model_2d_{:06d}".format(cur_iter), **checkpoint_data_2d
            )
            checkpoint_data_3d["iteration"] = cur_iter
            checkpoint_data_3d[best_metric_name] = best_metric["3d"]
            checkpointer_3d.save(
                "model_3d_{:06d}".format(cur_iter), **checkpoint_data_3d
            )
        ...
        if val_period > 0 and (cur_iter % val_period == 0 or cur_iter == max_iteration):
            ...
            logger.info(
                "Iteration[{}]-Val {}  total_time: {:.2f}s".format(
            ...
            for modality in ["2d", "3d"]:
                ...
                if cur_metric_name in val_metric_logger.meters:
                    ...
                    if (
                        best_metric[modality] is None
                        or best_metric[modality] < cur_metric
                    ):
    ...
    for modality in ["2d", "3d"]:
        logger.info(
            "Best val-{}-{} = {:.2f} at iteration {}".format(
                modality.upper(),
                cfg.VAL.METRIC,
