...
def parse_args():
    ...
    parser.add_argument(
        ...
        metavar="FILE",
        type=str,
        help="path to checkpoint file of the 2D model",
    )
    parser.add_argument(
        "--ckpt3d",
        metavar="FILE",
        type=str,
        help="path to checkpoint file of the 3D model",
    )
...
def predict(cfg, model_2d, model_3d, dataloader, pselab_path, save_ensemble=False):
    """
    ...
    with torch.no_grad():
        for iteration, data_batch in enumerate(dataloader):
            ...
            pred_label_voxel_ensemble = (
                (probs_2d + probs_3d).argmax(1).cpu().numpy() if model_3d else None
            )
            ...
            for batch_ind in range(len(points_idx)):
                ...
                pred_label_ensemble = (
                    pred_label_voxel_ensemble[left_idx:right_idx] if model_3d else None
                )
                ...
                pseudo_label_dict = {
                    "probs_2d": curr_probs_2d[range(len(pred_label_2d)), pred_label_2d]
                    .cpu()
                    .numpy(),
...
def test(cfg, args, output_dir="", summary_writer=None):
    ...
    if args.pselab:
        predict(
            cfg, model_2d, model_3d, test_dataloader, pselab_path, args.save_ensemble
        )
    else:
        validate(
            cfg,
